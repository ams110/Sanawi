...
**القرار المعتمد: نحسب نحن** (كمية × سعر من `/ticker` × سعر الصرف).
...
**واستثناءٌ واحد معلَن: Pionex.** ليس تساهلاً بل ضرورة — رأسمال بوتاتها لا
يظهر كمياتٍ منمَّطة في أيّ مسارٍ موثَّق، فحسابُه عندنا اختراعُ بنية، وقراءةُ
السبوت وحده تكذب بالنقصان صامتاً. التفصيل في قسمها أدناه، والإعلان في
الشيفرة نفسها (`usesExchangeTotal`) لا في هذا الملف وحده.

### مصدر الأسعار وسعر الصرف

| ما | من أين | لماذا |
|---|---|---|
| سعر العملة بالدولار | `GET api.binance.com/api/v3/ticker/price?symbols=[…]` (عامّ، بلا مفتاح) | أعمق سيولة وأشمل رموز؛ ومصدرٌ واحد يعني أن BTC سعرٌ واحد في كل المحافظ |
| سعر الدولار بالشيكل | `GET api.frankfurter.app/latest?from=USD&to=ILS` (بيانات البنك المركزي الأوروبي، بلا مفتاح ولا حدّ) | لا مفتاحَ يُدار ولا حصّةَ تنفد |

المرتبطة بالدولار (`USDT`، `USDC`، `DAI`…) سعرها واحدٌ بالتعريف: لا زوجَ
`USDTUSDT` أصلاً، وبلا هذه القائمة يسقط أكبر رصيدٍ في أغلب المحافظ صامتاً.
...
### التوقيع مُتحقَّقٌ منه بمثالهم لا بقراءتنا

صفحة Authentication تنشر مثالاً كاملاً: سرّاً وطابعاً ونصّاً والتوقيع الناتج.
مطابقتُه ليست تفصيلاً — هي الفرق بين «فهمنا النصّ» و«خادمُهم سيقبل». وهو
**مثبَّتٌ اختباراً** في `src/lib/crypto/sign.test.ts`، فأي تعديلٍ في سلسلة
التوقيع يكسره قبل أن يصل المنصّة.

وفيه درسٌ لولا المثال ما عُرف: **الجسم يُلحق بنصّ التوقيع** (مثالهم يلحقه حتى
في GET). فعقدُنا صار صريحاً: `payload` هو ما يُوقَّع حرفياً، ومن يبني طلباً
بجسمٍ يلحقه هناك ولا يفترض أن `signRequest` تجمعه عنه.

### الصلاحيات — ولماذا هذه المنصّة أأمن من غيرها هنا

الصلاحيات المنشورة: `Enable reading` · `Enable trading` · `Enable transfer` ·
`Bot reading` · `Bot trading` · `Earn (Beta)` · `Institutional deposit &
withdrawal`.

نحتاج **`Enable reading` وحدها**. والمهمّ: **لا توجد صلاحية سحبٍ للأفراد
أصلاً** — السحب الوحيد المذكور مؤسّسيٌّ داخلي. فأسوأ ما يفعله مفتاحٌ مسروق
بصلاحية القراءة أن يُقرأ الرصيد، لا أن يُفرَّغ.

### ⛔ Pionex.US لا تدعم مفاتيح API إطلاقاً

منصّةٌ مستقلّة عن `pionex.com` لمستخدمي أمريكا، ومركز مساعدتها يقول صراحةً إن
إنشاء المفاتيح **غير متاح** (المقال محدَّث كانون أول 2025، والبديل المعروض
تصدير الحركات يدوياً). فمن حسابه هناك لا يُربط بحال — لا مشكلة توقيعٍ تُصلَح.
