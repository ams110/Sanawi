 ...
 */
/*
 * أجسام الدوال ($$…$$) تُقشر قبل الفحص: `delete from` داخل دالةٍ لا يمسّ
 * صفاً عند لصق الملف — يُنفَّذ حين يستدعي المستخدمُ الدالةَ بقراره (فكُّ
 * ربط Financy يمسح مفاتيح صاحبها بطلبه، وهذا عملها لا عطبها). الحارس
 * يحرس لحظةَ الهجرة وحدها، وتعميمُه على الأجسام يحوّله من حارسٍ إلى مانعِ
 * كتابةِ دوالٍ مشروعة.
 */
const withoutFunctionBodies = (sql) => sql.replace(/\$\$[\s\S]*?\$\$/g, '$$…$$')

const FORBIDDEN = /\b(drop\s+table|truncate|delete\s+from)\b/i
const offending = files.filter((name) =>
  FORBIDDEN.test(withoutFunctionBodies(readFileSync(join(migrationsDir, name), 'utf8'))),
)
